...
def calculateFlowWithGPU(prvsFrame, nextFrame, method):
    '''Add more functionality to GPU based OF estimation. Right now just get dense working.'''
    flow=0
    if method=='Dense':
        print ("Running dense flow on gpu...")
        #gpu_frame = cv.cuda_GpuMat()
        #gpu_frame.upload(nextFrame)
        self.flow_algo = cv2.cuda_FarnebackOpticalFlow.create()
        gpu_current = cv.cuda.cvtColor(nextFrame, cv.COLOR_BGR2GRAY)
        #gpu_previous = cv.cuda_GpuMat()
        #gpu_previous.upload(prvsFrame)
        gpu_previous = cv.cuda.cvtColor(prvsFrame, cv.COLOR_BGR2GRAY)
        gpu_flow = cv.cuda_FarnebackOpticalFlow.create(5, 0.5, False, 15, 3, 5, 1.2, 0)
        gpu_flow = cv.cuda_FarnebackOpticalFlow.calc(gpu_flow, gpu_previous, gpu_current, None)
        flow = gpu_flow
    elif method=='nvidia2':
        nvof = cv2.cuda_NvidiaOpticalFlow_2_0.create(prvsFrame.shape[1], nextFrame.shape[0], 5, False, False, False, 0)
        flow = nvof.calc(prvsFrame, nextFrame, None)
    elif method=='nvidia1':
        nvof = cv2.cuda_NvidiaOpticalFlow_1_0.create(prvsFrame.shape[1], nextFrame.shape[0], 5, False, False, False, 0)
        flow = nvof.calc(prvsFrame, nextFrame, None)
    else:
...
def processFlow(export_dir, video_filename, maxFrameNum=9999, res=[512, 512], displayFlow=False, method='deepflow'):
    ...
    while frame2 is not None and imNum <= maxFrameNum:
        ...
        if method == 'deepflow':
            ...
            flow = calculateFlowWithGPU(prvsFrame, nextFrame, method) # change this to run on GPU
        else:
...
if __name__ == '__main__':
    ...
    if len(files) == 1:
        print ('starting flow processing function...')
        processFlow(export_dir, files[0], maxFrameNum=maxFrameNum,
         displayFlow=visualizeOF, res=res, method=method_)
    else:
